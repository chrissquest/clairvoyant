...
public class CornflowerWorldFeatures {
    ...
    public static final StructureFeature<?> RUIN_STRUCTURE = Registry.register(Registry.STRUCTURE_FEATURE, "cornflower:ruin_structure", RUIN_FEATURE);

    public static void init() {
        Feature.STRUCTURES.put("cornflower:ruin_structure", RUIN_FEATURE);

        for(Biome biome : Registry.BIOME)
        {
            if(biome.getCategory() != Biome.Category.OCEAN)
            {
                biome.addStructureFeature(RUIN_FEATURE.configure(FeatureConfig.DEFAULT));
                biome.addFeature(
